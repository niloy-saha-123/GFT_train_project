...
Usage (run from repo root: GFT_train_project/):
    python src/train_detector_mac2.py
    python src/train_detector_mac2.py --video data/raw/video_1.MOV
    python src/train_detector_mac2.py --video-dir data/raw
    python src/train_detector_mac2.py --no-display
    python src/train_detector_mac2.py --video /path/to/video.MOV --no-save
...
# ── OCR — match Colab v3: center 20–80%, gap 6; per-track gap so multiple cars read
OCR_CENTER_ZONE        = 0.20
OCR_FRAME_GAP          = 6      # v3: >= 6 frames between reads *per track id*
OCR_MIN_TEXT_LEN       = 3
OCR_MIN_CONF           = 0.40   # v3
OCR_SESSION_MAX_READS  = 250 # safety only (v3 had no cap)
...
class DirectionDetector:
    """
    Direction from **one sample per frame**: mean horizontal center of all boxes,
    then frame-to-frame motion votes (avoids multi-box-per-frame bias in the old
    deque). Tie-break with net displacement vs first frame after enough samples.
    """
    def __init__(self, frame_w: int):
        self.frame_w = frame_w
        self.direction       = None
        self._prev_mean_cx   = None
        self._first_mean_cx  = None
        self._vote_lr        = 0
        self._vote_rl        = 0
        self._frames         = 0

    def update_from_detections(self, dets: list) -> Optional[str]:
        if not dets:
            return self.direction
        cxs = sorted((d['x1'] + d['x2']) / 2.0 for d in dets)
        mean_cx = cxs[len(cxs) // 2]
        if self._first_mean_cx is None:
            self._first_mean_cx = mean_cx
        if self._prev_mean_cx is not None:
            dcx = mean_cx - self._prev_mean_cx
            if dcx > 1.2:
                self._vote_lr += 1
            elif dcx < -1.2:
                self._vote_rl += 1
        self._prev_mean_cx = mean_cx
        self._frames += 1

        if self.direction is None:
            if self._vote_lr >= self._vote_rl + 2:
                self.direction = 'L→R'
            elif self._vote_rl >= self._vote_lr + 2:
                self.direction = 'R→L'
            elif self._frames >= 10:
                net = mean_cx - self._first_mean_cx
                if net > 8:
                    self.direction = 'L→R'
                elif net < -8:
                    self.direction = 'R→L'
                elif self._frames >= 24:
                    if net >= 0:
                        self.direction = 'L→R'
                    else:
                        self.direction = 'R→L'
...
class TrainOCR:
    """
    Colab v3 OCR path: crop 65% height, resize if short, sharpen, readtext only.
    Per-track frame gap (same6-frame rule as v3, but per ByteTrack id) so many
    units can be read in the same wall-clock window.
    """
    ...
    def __init__(self, frame_w: int, frame_h: int):
        ...
        self.company              = None
        self.conf                 = 0.0
        self._last_ocr_by_tid     = {}
        self._session_read_count  = 0

    def should_run(self, cx: float, frame_num: int, track_id) -> bool:
        if self._session_read_count >= OCR_SESSION_MAX_READS:
            return False
        if not (self.ocr_x1 < cx < self.ocr_x2):
            return False
        tid = track_id if track_id is not None else -1
        last = self._last_ocr_by_tid.get(tid, -999)
        if frame_num - last < OCR_FRAME_GAP:
            return False
        return True
    ...
    def run(
        ...
        # v3 sets last-ocr frame at start of run (even if crop empty)
        tid = track_id if track_id is not None else -1
        self._last_ocr_by_tid[tid] = frame_num
        ...
        try:
            ...
            return [txt.strip().lower()
                    ...
                    if conf > OCR_MIN_CONF and len(txt.strip()) >= OCR_MIN_TEXT_LEN]
        except Exception:
            return []
...
class Renderer:
    ...
    def draw(self, frame, dets, counter, ocr, speed_mph, speed_kmh,
        ...
        lines = [
            ...
            f"OCR reads : {ocr._session_read_count}/{OCR_SESSION_MAX_READS}",
        ]
...
class TrainSession:
    ...
    def finalise(self, fps: float) -> dict:
        ...
        self.result = {
            ...
            'ocr_calls':          self.ocr._session_read_count,
        }
        return self.result
    ...
    while True:
        ...
        if active or state in ('TRAIN_IN_FRAME', 'TRAIN_GONE'):
            ...
            if dets:
                ...
                session.dir_det.update_from_detections(dets)
                session.counter.update(dets, frame_num)

                for d in dets:
                    cx = (d['x1'] + d['x2']) / 2.0
                    tid = d.get('track_id')
                    if session.ocr.should_run(cx, frame_num, tid):
